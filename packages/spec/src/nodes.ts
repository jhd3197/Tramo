...
export const BUILTIN_NODES: NodeDefinition[] = [
  ...
  /* ---------- utility nodes — bytes, encoding, signatures, parsing ---------- */
  {
    id: 'verify-signature',
    name: 'Verify Signature',
    category: 'transform',
    description: 'Compute an HMAC signature over the request body and compare it to a header. Pass/fail routes.',
    icon: 'ShieldCheck',
    color: COLORS.transform,
    inputs: [{ key: 'in', label: 'In', type: 'object' }],
    outputs: [
      { key: 'ok', label: 'Verified', type: 'object' },
      { key: 'bad', label: 'Bad signature', type: 'object' },
      { key: 'error', label: 'Error', type: 'object' },
    ],
    fields: [
      {
        ...
        options: [
          ...
          { label: 'Slack (X-Slack-Signature + X-Slack-Request-Timestamp)', value: 'slack' },
          { label: 'Twilio (X-Twilio-Signature)', value: 'twilio' },
        ],
        help: 'Picks the header name + algorithm + signing format for common providers.',
      },
      { key: 'secret', type: 'secret', label: 'Signing secret' },
      {
        key: 'algorithm',
        type: 'select',
        label: 'Algorithm (generic only)',
        default: 'sha256',
        options: [
      ...
      { key: 'headerName', type: 'text', label: 'Header name (generic only)', default: 'x-signature', optional: true },
      { key: 'bodySource', type: 'text', label: 'Body source expression', default: 'input.body', help: 'JS expression that evaluates to the raw body string the signature was computed over.', optional: true },
    ],
  },
  {
    id: 'csv-parse',
    name: 'CSV Parse',
    category: 'transform',
    description: 'Parse a CSV string into an array of row objects (or arrays).',
    icon: 'Table',
    color: COLORS.transform,
    inputs: [{ key: 'in', label: 'In', type: 'any' }],
    outputs: [
    ...
    fields: [
      { key: 'source', type: 'text', label: 'Source expression', default: 'input', help: 'JS expression that evaluates to the CSV string.', optional: true },
      { key: 'delimiter', type: 'text', label: 'Delimiter', default: ',', optional: true },
      { key: 'headerRow', type: 'boolean', label: 'First row is header', default: true, optional: true, help: 'When true, returns objects keyed by header. When false, returns arrays.' },
      { key: 'trim', type: 'boolean', label: 'Trim cells', default: true, optional: true },
    ],
  },
  {
    id: 'csv-stringify',
    name: 'CSV Stringify',
    category: 'transform',
    description: 'Render an array of objects (or arrays) into a CSV string.',
    icon: 'Table',
    color: COLORS.transform,
    inputs: [{ key: 'in', label: 'Rows', type: 'array' }],
    outputs: [{ key: 'out', label: 'CSV', type: 'string' }],
    fields: [
      { key: 'delimiter', type: 'text', label: 'Delimiter', default: ',', optional: true },
      { key: 'header', type: 'boolean', label: 'Emit header row', default: true, optional: true, help: 'Inferred from the first object\'s keys, in insertion order.' },
      { key: 'columns', type: 'json', label: 'Explicit columns (JSON array)', default: '[]', optional: true, help: 'When set, overrides header inference and orders columns exactly.' },
    ],
  },
  {
    id: 'fetch-binary',
    name: 'Fetch Binary',
    category: 'action',
    description: 'Download a URL as bytes and emit { base64, mimeType, size } so the body can be re-uploaded or attached downstream.',
    icon: 'Download',
    color: COLORS.action,
    inputs: [{ key: 'in', label: 'In', type: 'any' }],
    outputs: [
    ...
    fields: [
      { key: 'url', type: 'url', label: 'URL', default: 'https://example.com/file.pdf' },
      { key: 'headers', type: 'json', label: 'Headers (JSON)', default: '{}', optional: true },
      { key: 'timeoutMs', type: 'number', label: 'Timeout (ms)', default: 30000, optional: true },
    ],
  },
  {
    id: 'upload-binary',
    name: 'Upload Binary',
    category: 'action',
    description: 'POST a base64-encoded body to a URL as a multipart upload — pair with Fetch Binary for cross-service file transfers.',
    icon: 'Upload',
    color: COLORS.action,
    inputs: [{ key: 'in', label: 'In', type: 'any' }],
    outputs: [
    ...
    fields: [
      ...
      { key: 'fieldName', type: 'text', label: 'Form field name', default: 'file', optional: true },
      { key: 'fileName', type: 'text', label: 'File name (supports {{var}})', default: 'upload.bin', optional: true },
      { key: 'contentBase64', type: 'textarea', label: 'Content (base64, supports {{var}})', default: '' },
      { key: 'mimeType', type: 'text', label: 'MIME type', default: 'application/octet-stream', optional: true },
      { key: 'headers', type: 'json', label: 'Extra headers (JSON)', default: '{}', optional: true },
    ],
  },
  {
    id: 'encode',
    name: 'Encode / Decode',
    category: 'transform',
    description: 'Base64, hex, or URL-encode (or decode) a string.',
    icon: 'Binary',
    color: COLORS.transform,
    inputs: [{ key: 'in', label: 'In', type: 'any' }],
    outputs: [{ key: 'out', label: 'Result', type: 'string' }],
    fields: [
      { key: 'source', type: 'text', label: 'Source expression', default: 'input', help: 'JS expression evaluating to the string.', optional: true },
      {
        key: 'mode',
        type: 'select',
        label: 'Mode',
  ...
  },
  {
    ...
    name: 'Hash',
    category: 'transform',
    description: 'Compute a cryptographic hash (SHA-256 / SHA-1 / MD5) over a string.',
    icon: 'Fingerprint',
    color: COLORS.transform,
    inputs: [{ key: 'in', label: 'In', type: 'any' }],
    outputs: [{ key: 'out', label: 'Digest', type: 'string' }],
    fields: [
      { key: 'source', type: 'text', label: 'Source expression', default: 'input', help: 'JS expression evaluating to the string.', optional: true },
      {
        key: 'algorithm',
  ...
  },
  {
    ...
    name: 'Markdown → HTML',
    category: 'transform',
    description: 'Render a Markdown string into HTML. Handles headers, bold/italic, links, code, lists, and line breaks.',
    icon: 'FileCode',
    color: COLORS.transform,
    inputs: [{ key: 'in', label: 'In', type: 'any' }],
    outputs: [{ key: 'out', label: 'HTML', type: 'string' }],
    fields: [
      { key: 'source', type: 'text', label: 'Source expression', default: 'input', help: 'JS expression evaluating to the markdown string.', optional: true },
      { key: 'wrapInP', type: 'boolean', label: 'Wrap paragraphs in <p>', default: true, optional: true },
    ],
  },
  {
    id: 'html-to-text',
    name: 'HTML → Text',
    category: 'transform',
    description: 'Strip tags, decode entities, and collapse whitespace to produce plain-text from HTML.',
    icon: 'AlignLeft',
    color: COLORS.transform,
    inputs: [{ key: 'in', label: 'In', type: 'any' }],
    outputs: [{ key: 'out', label: 'Text', type: 'string' }],
    fields: [
      { key: 'source', type: 'text', label: 'Source expression', default: 'input', help: 'JS expression evaluating to the HTML string.', optional: true },
      { key: 'preserveLineBreaks', type: 'boolean', label: 'Preserve line breaks', default: true, optional: true, help: 'Convert <br>, </p>, and block elements to newlines instead of spaces.' },
    ],
  },
